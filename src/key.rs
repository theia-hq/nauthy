...
/// [`TAG`] followed by the base32-lowercase key body, identical to a `bifrost_core::NodeId` for the same
/// key, so a `sheer:` link is portable across the nauthy/bifrost boundary with no conversion at the wire.
