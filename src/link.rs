//! A capability link: the shareable wire form of a [`Cap`], parsed and verified at construction.
...
impl Link {
    /// Mint a link granting `service`, valid for `lifetime` from now.
    ///
    ...
    /// Mint a device-bound link granting `service` to the proven device `bound_to`, valid for
    /// `lifetime` from now.
    ///
    ...
    /// The exact link text, as parsed or minted: what [`Display`](fmt::Display) renders and a caller
...
impl FromStr for Link {
    ...
    /// Parse-don't-validate at the wire edge: rejects text that is not `<key>.<token>`, a malformed root key, bad base32, or a
...
impl fmt::Debug for Link {
    /// Print the link text: a link is public share material, not a secret, and its whole identity
